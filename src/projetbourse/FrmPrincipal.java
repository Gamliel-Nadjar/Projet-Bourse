...
public class FrmPrincipal extends javax.swing.JFrame {
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        Action act6 = new Action(6, "VMWare", 121.56, 123, 43);
        Action act7 = new Action(7, "IBM", 42.61, 40.98, 126);
        
        //on ajoute au trader les action
...
//        for(Trader trad : mesTraders)
//        {
//            //int numTrader = parseInt(tblTraders.getValueAt(tblTraders.getSelectedRow(),0).toString());
//            if(trad.getIdTrader() == parseInt(tblTraders.getValueAt(tblTraders.getSelectedRow(),0).toString()))
//            {
//                
//
//        
...
//                
//                   // numTrader ++;
//                   // montantPortefeuille = (coursReel * quantiteAcheter);  
//                   // montantPortefeuille = (montantPortefeuille - (prixAchat * quantiteAcheter))+montantPortefeuille;
//                    montantPortefeuille = (double) (((act.getCoursReel() * act.getQuantiteAcheter()) - (act.getPrixAchat() * act.getQuantiteAcheter()))+montantPortefeuille);
//               }
//            }
//        }
//        
//        //double res = (double)((coursReel * quantiteAcheter) - (prixAchat * quantiteAcheter));
...
//        
    ...
    private void tblTradersMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblTradersMouseClicked
        ...
        // A vous de jouer
     //   (cours réel * quantité achetée) – (prix achat * quantité achetée)
    // Lors d’un clique sur le JTABLE, on affiche les actions du trader sélectionné ainsi que le montant de son portefeuille.
//    
//        int coursReel = 0;
//        int quantiteAcheter = 0;
//        int prixAchat =0;
...
//        }
       ...
       // Vector v;
        ...
        String choix = tblTraders.getValueAt(tblTraders.getSelectedRow(),1).toString();
        for(Trader trad : mesTraders)    
         {
             if(trad.getIdTrader() == parseInt(tblTraders.getValueAt(tblTraders.getSelectedRow(),0).toString()))
             for(Action act : trad.getmesActions())
...
//        Lors d’un clique sur une action, on affiche si le trader gagne ou perd de l’argent sur l’action sélectionnée.
//
//Si le cours réel de l’action est supérieur au prix achat  le trader gagne de l’argent sinon il en perd.
        double montant = 0;
        for(Trader trad : mesTraders)
            for(Action act : trad.getmesActions())
            {
              if(trad.getIdTrader() > 0)
              {
                  //JOptionPane.showMessageDialog(this, "Vous gagnez de l'argent");
                  montant = (double) (act.getCoursReel() - act.getPrixAchat());
                  boolean montantAction = montant > 0;
                
              }
              else
              {
                  //JOptionPane.showMessageDialog(this, "Vous perdez de l'argent");
                  montant = (double) (act.getCoursReel() - act.getPrixAchat());
                  boolean montantAction = montant < 0;
              }
            
            } lblMessage.setText(String.valueOf(montant).concat("Vous gagnez de l'argent"));
